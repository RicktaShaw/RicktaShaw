# About Me:
## Rickta Shaw  
[![LinkedIn](https://img.shields.io/badge/LinkedIn-%230077B5.svg?logo=linkedin&logoColor=white)](YOUR_LINKEDIN_LINK) 
[![Email](https://img.shields.io/badge/Email-D14836?logo=gmail&logoColor=white)](mailto:YOUR_EMAIL)

I am an Engineer at Tata Consultancy Services (TCS) with 1 year of hands-on experience in developing and supporting internal enterprise tools and applications. My professional journey has enabled me to build a strong foundation in programming, web technologies, and database management systems while working in a structured enterprise environment.  

<br><br>

My technical expertise includes Java, C, and Python, along with frontend technologies such as HTML, CSS, JavaScript, and Bootstrap. I am proficient in SQL, MySQL, and core DBMS concepts, enabling me to design and manage structured, data-driven solutions. I regularly use Git, GitHub, VS Code, and Eclipse to ensure efficient development workflows and version control best practices.  

<br><br>

I am particularly interested in Data Analytics and Full Stack Development. Currently, I am strengthening my skills in Advanced SQL, data handling, and end-to-end web application development. I enjoy solving real-world problems, writing clean and maintainable code, and building scalable solutions aligned with business objectives. Driven by consistency, discipline, and continuous learning, I aim to grow into a versatile Full Stack Developer and Data Analyst.  

---

### Tech Stack:

![C](https://img.shields.io/badge/c-%2300599C.svg?style=for-the-badge&logo=c&logoColor=white) ![CSS3](https://img.shields.io/badge/css3-%231572B6.svg?style=for-the-badge&logo=css3&logoColor=white) ![Java](https://img.shields.io/badge/java-%23ED8B00.svg?style=for-the-badge&logo=openjdk&logoColor=white) ![HTML5](https://img.shields.io/badge/html5-%23E34F26.svg?style=for-the-badge&logo=html5&logoColor=white) ![JavaScript](https://img.shields.io/badge/javascript-%23323330.svg?style=for-the-badge&logo=javascript&logoColor=%23F7DF1E) ![Python](https://img.shields.io/badge/python-3670A0?style=for-the-badge&logo=python&logoColor=ffdd54) ![Azure](https://img.shields.io/badge/azure-%230072C6.svg?style=for-the-badge&logo=microsoftazure&logoColor=white) ![Bootstrap](https://img.shields.io/badge/bootstrap-%238511FA.svg?style=for-the-badge&logo=bootstrap&logoColor=white) ![MySQL](https://img.shields.io/badge/mysql-4479A1.svg?style=for-the-badge&logo=mysql&logoColor=white) ![Figma](https://img.shields.io/badge/figma-%23F24E1E.svg?style=for-the-badge&logo=figma&logoColor=white) ![Canva](https://img.shields.io/badge/Canva-%2300C4CC.svg?style=for-the-badge&logo=Canva&logoColor=white) ![Flask](https://img.shields.io/badge/flask-%23000.svg?style=for-the-badge&logo=flask&logoColor=white)

---

## Experience:

## Tata Consultancy Services (TCS)  
**Role:** Engineer  
**Duration:** 2025 – Present  

### Key Responsibilities
- Developing and supporting internal enterprise tools and applications.  
- Troubleshooting, debugging, and optimizing system performance.  
- Maintaining structured and efficient database-driven solutions.  
- Collaborating with cross-functional teams to ensure smooth application functionality.  
- Following SDLC best practices and maintaining documentation.  

### Impact
- Contributing to stable and reliable internal systems.  
- Supporting seamless business operations through timely issue resolution.  

---

## Education:

**Techno India Kolkata (MAKAUT)**  
**Bachelor of Computer Applications (BCA)**  
*2021 – 2024*  
**CGPA:** 8.79  

**Khardah Priyanath Balika Vidyalaya (WBCHSE)**  
**Higher Secondary Examination**  
*2021*  

**Khardah Priyanath Balika Vidyalaya (WBBSE)**  
**Madhyamik Examination**  
*2019*  

---

# GitHub Stats:

![](https://github-readme-stats.vercel.app/api?username=YOUR_GITHUB_USERNAME&theme=dark&hide_border=false)
<br/>
![](https://nirzak-streak-stats.vercel.app/?user=YOUR_GITHUB_USERNAME&theme=dark&hide_border=false)
<br/>
![](https://github-readme-stats.vercel.app/api/top-langs/?username=YOUR_GITHUB_USERNAME&theme=dark&layout=compact)

---
